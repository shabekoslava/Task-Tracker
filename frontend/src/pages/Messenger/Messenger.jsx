...
export default function Messenger({
  ...
  useEffect(() => {
    ...
    if (latestMsgs.length > 0) {
      const newNotifs = latestMsgs.map((item) => ({
        id: item.msg.id,
        date: new Date(item.msg.createdAt).toLocaleDateString([], {
          day: "2-digit",
          month: "2-digit",
        }),
        time: new Date(item.msg.createdAt).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
      ...
      setRecentNotifications((prev) => {
        const combined = [...newNotifs, ...prev].filter(
  ...
  const allRegisteredUsers = JSON.parse(
    ...
    Object.values(tasksObj).forEach((task) => {
      if (task.assignedTo === currentUserId) {
        const taskMessages = (task.comments || []).map((c) => ({
          ...
          status: "read",
          isComment: !c.id.includes("comment-msg-"),
        }));
    ...
    if (uniqueRoomsMap.has(storedRoom.id)) {
      ...
      if (!roomName) {
        if (storedRoom.type === "personal") {
          const otherId = Array.isArray(storedRoom.members)
            ? storedRoom.members.find((m) => m !== currentUserId)
            : null;
          roomName = `👤 ${getUserName(otherId)}`;
        } else if (storedRoom.type === "group") {
          const names = Array.isArray(storedRoom.members)
            ? storedRoom.members
  ...
  const accessibleRooms = allRooms.filter((room) => {
    if (room.type === "project" || room.type === "task") {
      ...
      return Array.isArray(proj.members)
        ? proj.members.some((m) => m.id === currentUserId)
        : false;
    }
  ...
  // Create personal chat (by teammate click or manual ID search)
  const handleCreatePersonalChat = (teammateId) => {
    ...
    if (exists) {
      ...
      // Look up the user in ALL registered users, not just teammates
      const foundUser = allRegisteredUsers.find((u) => u.id === teammateId);
      const teammateName = foundUser?.name || teammateId;
      const newRoom = {
        id: roomId,
        name: `👤 ${teammateName}`,
        type: "personal",
  ...
  const [personalSearchId, setPersonalSearchId] = useState("");
  ...
  const handleCreateGroupChat = () => {
    ...
    if (selectedGroupMembers.length === 0) {
      alert("Выберите хотя бы одного участника для создания группы.");
      return;
    }
      ...
      prev.map((r) => {
        ...
        return {
          ...
          members: Array.isArray(r.members)
            ? r.members.filter((m) => m !== currentUserId)
            : [],
        };
      }),
    );
    ...
    <div className="messenger-workspace page-fade-in">
      ...
      <aside className="messenger-sidebar">
        ...
        <div className="sidebar-channels-scroll">
          ...
          <div
            className="channel-category-block"
                  ...
                  .map((room) => (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{
                          fontWeight: hasUnread(room) ? "bold" : "normal",
                        }}
                      >
                        {room.name.replace("👤 ", "")}
                      </span>
              ...
              >
                {userProjects.map((project) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="project-task-group-title"
                        style={{
                          fontSize: "11px",
                          fontWeight: "700",
        ...
        {activeRoom ? (
          <>
            <header className="chat-header">
                  ...
                  {activeRoom.type === "task" &&
                    (() => {
                      const proj = projects.find(
                        (p) => p.id === activeRoom.projectId,
                      );
                      return proj ? (
                        <span
                          className="room-meta-tag"
                          style={{
              ...
              <div
                className="chat-context-actions"
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (userId && userId.trim()) {
                        ...
                        if (activeRoom.members?.includes(trimmedId)) {
                        ...
                        setChats((prev) =>
                          prev.map((r) => {
                            ...
                            return {
                              ...r,
                              members: [...(r.members || []), trimmedId],
                              messages: [
                                ...(r.messages || []),
                                {
                                  id: `msg-${Date.now()}`,
                                  text: `Пользователь ${foundUser.name} добавлен в группу.`,
                                  createdAt: new Date().toISOString(),
                                  authorId: "System",
                                  status: "read",
                                },
            ...
            <div className="chat-messages-thread">
              {currentMessages.length === 0 ? (
                ...
                currentMessages.map((msg) => {
                  ...
                  return (
                    <div
                      className={`message-bubble-wrapper ${isOwn ? "own-message" : ""}`}
                      key={msg.id}
                    >
                      <div
                      ...
                      >
                        {getUserAvatar(msg.authorId) ? (
                          <img
                            src={getUserAvatar(msg.authorId)}
                            alt="avatar"
                            style={{
                              width: "100%",
                              height: "100%",
                      ...
                      <div className="message-content-block">
                        ...
                        <div
                          className={`message-bubble-text ${msg.isComment ? "comment-aggregate" : ""}`}
                        >
                          {msg.text}
                          {msg.isComment && (
                            <span className="comment-sync-badge">
                              синхронизировано из карточки
                            </span>
                          )}
                        </div>
                        {isOwn && (
          ...
          <div className="chat-empty-state-full">
            ...
            <p>
              Выберите любой проектный или личный чат из боковой панели для
              начала переписки.
            </p>
          </div>
        )}
      ...
      <aside className="messenger-tech-panel">
        ...
        <section
          className="tech-block notifications-inbox-section"
          style={{
            flex: 1,
            display: "flex",
            flexDirection: "column",
        ...
        >
          ...
          >
            <div className="modal-header">
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ textAlign: "left" }}>
                  ...
                  <span
                    style={{ fontSize: 11, color: "var(--nav-text-inactive)" }}
                  >
            ...
            <div
              ...
              style={{ maxHeight: "50vh", overflowY: "auto", paddingRight: 6 }}
            >
              {recentNotifications.length === 0 ? (
                <div
                  style={{
            ...
            <div className="modal-body">
              ...
              >
                <input
                  ...
                  value={personalSearchId}
                  onChange={(e) => setPersonalSearchId(e.target.value)}
                  style={{
                    flex: 1,
                    padding: "8px 12px",
                    border: "1px solid var(--border-color)",
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && personalSearchId.trim()) {
              ...
              {activeTeammates.length === 0 ? (
                <p className="no-data-text">
                  Среди ваших коллег пока никого нет.
                </p>
              ) : (
